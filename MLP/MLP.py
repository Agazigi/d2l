...
# inputs --> hiddens
W1 = nn.Parameter(th.randn(num_inputs, num_hiddens, requires_grad=True) * 0.01)
b1 = nn.Parameter(th.zeros(num_hiddens, requires_grad=True))

# hiddens --> outputs
W2 = nn.Parameter(th.randn(num_hiddens, num_outputs, requires_grad=True) * 0.01)
b2 = nn.Parameter(th.zeros(num_outputs, requires_grad=True))

params = [W1, b1, W2, b2]

def relu(X):
    return th.max(th.zeros_like(X), X)

def net(X):
    X = X.reshape((-1, num_inputs))
    H = relu(th.matmul(X, W1) + b1)
    return th.matmul(H, W2) + b2

loss = nn.CrossEntropyLoss(reduction='none')

num_epochs, lr = 10, 0.1

updater = th.optim.SGD(params, lr)
